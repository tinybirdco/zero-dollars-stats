    ...
    >
      {KPI_OPTIONS.map(({ label, value, formatter }) => (
        <button
          ...
          className="relative cursor-pointer p-6 md:p-6 text-left md:text-center text-secondary hover:bg-primaryLight transition-colors sm:border-b-4 sm:border-transparent sm:state-active:border-secondary sm:mb-2"
        ...
        >
          ...
          <div className="hidden sm:block arrow absolute h-3 w-3 bg-secondary -bottom-5" />
        </button>
      ))}
    </div>
  )
